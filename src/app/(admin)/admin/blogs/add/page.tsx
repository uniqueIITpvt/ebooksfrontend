...
import { blogsApi, type BlogPayload } from '@/services/api/blogsApi';
...
export default function AddBlogPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let response;
      if (imageFile) {
        response = await blogsApi.createBlogWithFiles(blogPayload, imageFile);
      } else {
        response = await blogsApi.createBlog(blogPayload);
      }

      if (response.success) {
        showSuccessAlert('Blog created successfully!');
        setTimeout(() => {
          router.push('/admin/blogs');
        }, 1500);
      } else {
        showErrorAlert(response.message || 'Failed to create blog');
      }
    } catch (error: any) {
      console.error('Error creating blog:', error);
      showErrorAlert(error.message || 'Failed to create blog');
    } finally {
      setSubmitting(false);
    }
  };
