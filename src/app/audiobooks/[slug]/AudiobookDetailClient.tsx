...
export default function AudiobookDetailClient({ audiobook }: AudiobookDetailClientProps) {
  ...
  const transcriptWords = useMemo(
    () => normalizeTimedWords(rawTranscriptWords, duration),
    [duration, rawTranscriptWords]
  );

  const transcriptDisplayWords = useMemo(
    () => transcriptWords.map((word) => word.text),
    [transcriptWords]
  );
  const displayWords = useMemo(
    () => (transcriptDisplayWords.length ? transcriptDisplayWords : scriptWords),
    [scriptWords, transcriptDisplayWords]
  );
  const timingMethod = String(audiobook.wordSync?.method || '').toLowerCase();
  const audiobookLanguage = String(audiobook.language || '').toLowerCase();
  ...
  const estimatedWordTimings = useMemo(
    () => buildEstimatedWordTimings(displayWords, duration),
  ...
  const persistentTrackId = `audiobook-${audiobook.id || (audiobook as any)._id || audiobook.slug}`;
  ...
  const handleTogglePlay = async () => {
    ...
    if (isPersistentTrack && persistentAudio.isPlaying) {
    ...
    if (isPlaying && !isPersistentTrack) {
    ...
    await persistentAudio.playTrack({
      id: persistentTrackId,
      title: audiobook.title,
      author: audiobook.author,
      image: audiobook.image,
  ...
  return (
    <div style={{ fontFamily: 'var(--font-dm-sans, "DM Sans"), var(--font-devanagari, "Noto Sans Devanagari"), sans-serif', background: THEME.pageBg, color: THEME.text, minHeight: '100vh', paddingTop: 0, overflowX: 'hidden' }}>
      ...
      {/* Body */}
      <div className="content-grid" style={{ maxWidth: '1400px', margin: '0 auto', padding: '24px 40px', display: 'grid', gridTemplateColumns: '1fr', gap: '28px' }}>
